...
- **Dockerfile**: The central artifact. Builds a Debian-based image with:
  - Node.js v24 (via nvm, manual install for multi-arch precision; Corepack + pnpm enabled)
  - AWS CLI v2
...
- **Use Docker CLI inside container**: `docker run --rm -it -v /var/run/docker.sock:/var/run/docker.sock cloud-ops-builder bash`
